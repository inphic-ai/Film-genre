...
- [ ] 新增標籤選擇器組件（TagSelector.tsx）
  - [ ] 多選下拉選單（使用 shadcn/ui Select）
  - [ ] 搜尋標籤功能
  - [ ] 依類型分組顯示（KEYWORD / PRODUCT_CODE）
  - [ ] 顯示標籤使用次數
  - [ ] 標籤上限驗證（最多 5 個）
- [ ] 整合到影片編輯表單
  - [ ] 新增「標籤」欄位
...
- [ ] 新增「建立新標籤」功能
  - [ ] 輸入標籤名稱
  - [ ] 自動檢測商品編號格式（英文3碼+數字6碼+a/b/c）
  - [ ] 符合格式自動設為 PRODUCT_CODE，否則為 KEYWORD
  - [ ] 提供手動切換類型按鈕（管理員專用）
  - [ ] 輸入標籤描述與顏色
...
- [ ] 管理員標籤編輯介面
  ...
  - [ ] 即時更新標籤列表（無需重新整理）
- [ ] 標籤顯示優化
  - [ ] PRODUCT_CODE 標籤顯示特殊圖示（📦）
  - [ ] 標籤依權重排序顯示
  - [ ] 標籤 hover 顯示詳細資訊（使用次數、描述）
- [ ] 權限控制
  - [ ] 只有管理員可以編輯標籤
  - [ ] 一般使用者只能檢視標籤
